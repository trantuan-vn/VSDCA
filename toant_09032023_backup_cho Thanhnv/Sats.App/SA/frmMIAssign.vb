...
#Region "Method"
    ...
    Private Sub LoadUserInterface(ByRef pv_ctrl As Windows.Forms.Control)
        ...
        Try
            ...
            Me.Text = mv_ResourceManager.GetString("frmMIAssign")
            Dim v_strCaption As String
            If AssignType = "User" Then
                v_strCaption = Replace(mv_ResourceManager.GetString("lbCaption"), "$", "NSD")
            Else
                v_strCaption = Replace(mv_ResourceManager.GetString("lbCaption"), "$", " nhóm")
            End If
    ...
    Private Sub FillData()
        ...
        Try
            ...
            v_strCmdInquiryUsrOutGrp = "SELECT   micode VALUE, micode || ' - ' || NAME display" _
                                            & " FROM rgmi WHERE micode NOT IN (SELECT micode FROM tlmemauth" _
                                            & " WHERE AUTHID = '" & GroupId & "' AND authtype = 'G')" _
                                            & " And deleted = 0 ORDER BY micode"
            ...
            v_strCmdInquiryUserInGroup = "SELECT   micode VALUE, micode || ' - ' || NAME display" _
                                            & " FROM rgmi WHERE micode IN (SELECT micode FROM tlmemauth" _
                                            & " WHERE AUTHID = '" & GroupId & "' AND authtype = 'G')" _
                                            & " And deleted = 0 ORDER BY micode"
    ...
    Private Sub FillTellerData()
        ...
        Try
            ...
            v_strUsrOutGrpObjMsg = BuildXMLObjMsg(Now.Date, m_BusLayer.CurrentTellerProfile.BranchId, Now.Date, _
                    ...
                    OBJNAME_SY_AUTHENTICATION, CommonLibrary.gc_ActionInquiry, , m_BusLayer.CurrentTellerProfile.TellerId, "GetTellerMIData")
    ...
    Private Sub OnSave()
        ...
        Try
            ...
            If AssignType = "Group" Then
                v_strObjMsg = BuildXMLObjMsg(, , , TellerId, gc_IsNotLocalMsg, gc_MsgTypeObj, OBJNAME_SA_TLGROUPS, gc_ActionAdhoc, , v_strClause, "MIAssign")
            Else
                v_strObjMsg = BuildXMLObjMsg(, , , TellerId, gc_IsNotLocalMsg, gc_MsgTypeObj, OBJNAME_SA_TLPROFILES, gc_ActionAdhoc, , v_strClause, "MIAssign")
            End If
