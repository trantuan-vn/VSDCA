...
#Region "Khai báo"
    ...
    Private mv_blnBrStatus As Boolean
...
#Region "Method"
    ...
    Public Sub OnInit()
        Try
            ...
            End If
            'BangPV Chuyển:để hiển thị chứng khoán trong chi nhánh đã được phân quyền
            'If AssignType = "Group" Then
            '    FillData()
            'Else
            '    FillUserData()
    ...
    Private Sub LoadUserInterface(ByRef pv_ctrl As Windows.Forms.Control)
        ...
        Try
            ...
            Me.Text = mv_ResourceManager.GetString("frmStockAssign")
            Dim v_strCaption As String
            If AssignType = "User" Then
                v_strCaption = Replace(mv_ResourceManager.GetString("lbCaption1"), "$", "NSD")
            Else
                v_strCaption = Replace(mv_ResourceManager.GetString("lbCaption1"), "$", " nhóm")
            End If
            lbCaption.Text = v_strCaption & GroupName
        ...
        Try
            ...
            Else
                v_strCmdInquiryUserInGroup = "SELECT   brid VALUE, brid || ' - ' || brname display" _
                                                ...
                                                & " WHERE a.AUTHID = '" & GroupId & "' " _
                                                & " OR (b.tlid = '" & GroupId & "' and b.grpid =a.authid)" _
                                                & " )" _
                                                & " And deleted = 0 ORDER BY brid"
            ...
            Dim v_lngError As Long = Proxy.Message(v_strUserInGroupObjMsg)
            If v_lngError <> ERR_SYSTEM_OK Then
                MsgBox(IIf(UserLanguage = gc_LANG_VIETNAMESE, gc_ERR_GET_MSG_VN, gc_ERR_GET_MSG_EN), MsgBoxStyle.Critical, gc_ApplicationTitle)
                Exit Sub
            End If
    ...
    Private Sub FillData(ByVal v_strBrID As String)
        ...
        Try
            ...
            'bangpv
            'Thêm: lọc hiển thị chứng khoán theo chi nhánh mà nhóm có quyền
            v_strCmdInquiryUsrOutGrp = "SELECT   sicode VALUE, sicode || ' - ' || STOCK_NAME display" _
                                            & " FROM rgsi WHERE sicode NOT IN (SELECT sicode FROM tlstockauth" _
                                            & " WHERE AUTHID = '" & GroupId & "' AND authtype = 'G')" _
                                            & " and brid = '" & v_strBrID & "'" _
                                            & " And deleted = 0 and status=0 ORDER BY sicode"
            ...
            'bangpv
            'Thêm: lọc hiển thị chứng khoán theo chi nhánh mà nhóm có quyền
            v_strCmdInquiryUserInGroup = "SELECT   sicode VALUE, sicode || ' - ' || STOCK_NAME display" _
                                            & " FROM rgsi WHERE sicode IN (SELECT sicode FROM tlstockauth" _
                                            & " WHERE AUTHID = '" & GroupId & "' AND authtype = 'G')" _
                                            & " and brid = '" & v_strBrID & "'" _
                                            & " And deleted = 0 and status=0 ORDER BY sicode"
            v_strUserInGroupObjMsg = BuildXMLObjMsg(, , , TellerId, gc_IsLocalMsg, gc_MsgTypeObj, OBJNAME_SA_TLPROFILES, gc_ActionInquiry, v_strCmdInquiryUserInGroup)
    ...
    Private Sub FillUserData(ByVal v_strBrID As String)
        ...
        Try
            ...
            'Lay TVLK chua dc phan quyen cho nhom hoac NSD
            'bangpv
            'Thêm: lọc hiển thị chứng khoán theo chi nhánh mà user có quyền
            v_strCmdInquiryUsrOutGrp = "SELECT sicode VALUE, sicode || ' - ' || STOCK_NAME display" _
                                            & " FROM rgsi WHERE sicode NOT IN(" _
                                            & " SELECT  DISTINCT m.sicode" _
                                            & " FROM rgsi m, tlstockauth a" _
                                            ...
                                            & " And m.deleted = 0 and m.status=0 And m.sicode = a.sicode) AND deleted=0 and status=0" _
                                            & " And brid = '" & v_strBrID & "'" _
                                            & " ORDER BY VALUE"
            ...
            ''''==== Select users' name that are in group ====''''
            'Lay TVLK da phan quyen cho nhom hoac NSD
            v_strCmdInquiryUserInGroup = "SELECT  DISTINCT m.sicode VALUE, m.sicode || ' - ' || STOCK_NAME display, a.authtype" _
                                        & " FROM rgsi m, tlstockauth a" _
                                        & " WHERE ((a.AUTHID = '" & GroupId & "' AND a.authtype = 'U')" _
                                        & " OR (a.AUTHID IN (SELECT grpid FROM tlgrpusers" _
                                        & " WHERE tlid = '" & GroupId & "') AND a.authtype = 'G'))" _
                                        & " And m.deleted = 0 And m.sicode = a.sicode" _
                                        & " And m.brid = '" & v_strBrID & "'" _
                                        & " ORDER BY M.sicode"
            ...
            For i As Integer = 0 To v_nodeInGrpList.Count - 1
                ...
                If Not hTellerFilter(v_strSICode) Is Nothing Then
                    If v_strSICode <> v_strtmpSICode Then
                        lstUserInGroup.Items.Add(v_strDisplay)
                    End If
                    If v_strAuthType = "U" Then
                        mv_strMIInUser &= v_strSICode & "|"
                    Else
                        hTlidInGrpFilter.Add(v_strSICode, v_strSICode)
                    End If
                End If
                v_strtmpSICode = v_strSICode
            Next
    ...
    Private Sub FillTellerData()
        ...
        Try
            ...
            'Lay TVLK chua dc phan quyen cho nhom hoac NSD
            'v_strCmdInquiryUsrOutGrp = "SELECT sicode" _
            '                                & " FROM rgsi WHERE sicode NOT IN(" _
            '                                & " SELECT  DISTINCT m.sicode" _
            '                                & " FROM rgsi m, tlstockauth a" _
            '                                & " WHERE ((a.AUTHID = '" & GroupId & "' AND a.authtype = 'U')" _
            '                                & " OR (a.AUTHID IN (SELECT grpid FROM tlgrpusers" _
            '                                & " WHERE tlid = '" & GroupId & "') AND authtype = 'G'))" _
            '                                & " And m.deleted = 0 And m.sicode = a.sicode) AND deleted=0" _
            '                                & " ORDER BY VALUE"
            ...
            v_strUsrOutGrpObjMsg = BuildXMLObjMsg(Now.Date, m_BusLayer.CurrentTellerProfile.BranchId, Now.Date, _
                    ...
                    OBJNAME_SY_AUTHENTICATION, CommonLibrary.gc_ActionInquiry, , m_BusLayer.CurrentTellerProfile.TellerId, "GetTellerStockData")
    ...
    Private Sub OnSave()
        ...
        Try
            ...
            If AssignType = "Group" Then
                v_strObjMsg = BuildXMLObjMsg(, v_lstbrid_value, , TellerId, gc_IsNotLocalMsg, gc_MsgTypeObj, OBJNAME_SA_TLGROUPS, gc_ActionAdhoc, , v_strClause, "StockAssign")
            Else
                v_strObjMsg = BuildXMLObjMsg(, v_lstbrid_value, , TellerId, gc_IsNotLocalMsg, gc_MsgTypeObj, OBJNAME_SA_TLPROFILES, gc_ActionAdhoc, , v_strClause, "StockAssign")
            End If
    ...
    Private Sub ReMoveOne()
        Try
            ...
            If Not hTlidInGrpFilter(v_strSelect) Is Nothing Then
                MsgBox(mv_ResourceManager.GetString("RemoveErr1") & v_strSelect, MsgBoxStyle.Information + MsgBoxStyle.OkOnly, gc_ApplicationTitle)
                Exit Sub
            End If
        ...
        Try
            ...
            If AssignType = "User" Then
                ...
                If v_strMsg <> "" Then
                    ...
                    MsgBox(mv_ResourceManager.GetString("RemoveErr1") & v_strMsg, MsgBoxStyle.Information + MsgBoxStyle.OkOnly, gc_ApplicationTitle)
                End If
            End If
        Catch ex As Exception
            LogError.Write("Error source: " & ex.Source & vbNewLine _
...
#Region "Sự kiện Form"
    ...
    Private Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        If Not lstUserNoGroup.SelectedItems.Count < 1 Then
            Me.MoveOne()
        Else
            MsgBox(mv_ResourceManager.GetString("NothingSelected1"), MsgBoxStyle.Information + MsgBoxStyle.OkOnly, gc_ApplicationTitle)
        End If
    End Sub
...
#End Region
    ...
    Private Sub lstBrID_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lstBrID.SelectedIndexChanged
        Try
            Dim v_lstbrid_value As String
            v_lstbrid_value = lstBrID.SelectedItem.ToString.Split("-")(0).Trim
            If AssignType = "Group" Then
                FillData(v_lstbrid_value)
            Else
                FillUserData(v_lstbrid_value)
            End If
