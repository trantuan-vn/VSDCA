...
Public Class frmExportData
    ...
    Private v_blnExpSi, v_blnExpIs, v_blnExpMi, v_blnExpRoom, v_blnExpIInfo, v_blnTransRj, v_blnCacbonAllowance, v_blnCacbonCredit, v_blnExpIInfoCacbon, v_blnTransRjCacbon, v_blnExpMiCacbon As Boolean
    ...
    'TPRL 
    Private v_dCurrentDateTPRL As String = "_" & CStr(Format(Now.Date, "yyyyMMdd"))
    'end TPRL 
    Private v_dCurrentDateCacbon As String = "_" & CStr(Format(Now.Date, "yyyyMMdd"))
    Private v_strFileNHNN As String = "CDS_TRANS_NHNN" & v_dCurrentDate_NHNN & "_" & DateTime.Now.ToString("HH:mm:ss").Replace(":", "") & ".xml"
...
#Region "Method"
    ...
    Private Sub OnInit()
        ...
        Try
            ...
            'TPRL
            v_dCurrentDateTPRL = "_" & CStr(Format(v_dtIndex, "yyyyMMdd"))
            'end TPRL
            'end bangpv 
            v_dCurrentDateCacbon = "_" & CStr(Format(v_dtIndex, "yyyyMMdd"))
    ...
    End Function
    'Bond INFO TPRL
    Private Function GetXMLSIBondCData() As Xml.XmlDocument
        Try
            Dim v_strSQL As String = ""
            Dim v_xmlDocument As New Xml.XmlDocument
            Dim v_strObjMsg As String
            'a.interest_period_assume interest_period_unit, 'VND' CURRENCY 
            v_strSQL = "select a.sicode,a.isin,b.isname,  a.part_value PAR_VALUE,c.TOTAL_QTTY, to_char(a.ISSUER_DATE,'dd/MM/yyyy') ISSUE_DATE," _
                        & " a.bond_period PERIOD, a.bond_period_unit PERIOD_UNIT , to_char(a.DUE_DATE,'dd/MM/yyyy') DUE_DATE, " _
                        & "a.interest_type, a.interest_rate_type RATE_TYPE, a.interest_rate COUPON_RATE, a.interest_period, " _
                        & "3 interest_period_unit, 'VND' CURRENCY " _
            ...
            v_strObjMsg = BuildXMLObjMsg(Now.Date, m_BusLayer.CurrentTellerProfile.BranchId, Now.Date, _
                    m_BusLayer.CurrentTellerProfile.TellerId, CommonLibrary.gc_IsLocalMsg, CommonLibrary.gc_MsgTypeObj, _
                    OBJNAME_SY_AUTHENTICATION, CommonLibrary.gc_ActionInquiry, v_strSQL, "BONDS_INFO", "GetExportData")

            Dim v_strObjMsgTemp As String = v_strObjMsg
            v_strObjMsgTemp = Replace(v_strObjMsgTemp, " />", ">")
            Dim v_strXMLHeader As String = "<?xml version=""1.0"" standalone=""yes"" ?> "
            Dim v_strRootOpen = "<HNX_BONDS_CORP  xmlns=""http://tempuri.org/HNX_BONDS_CORP.xsd"">"
            Dim v_strRootClose = "</HNX_BONDS_CORP>"
    ...
    Private Function GetXMLRGIIInfoData() As Xml.XmlDocument
        Try
            Dim v_strSQL As String = ""
            Dim v_xmlDocument As New Xml.XmlDocument
            Dim v_strObjMsg As String

            v_strSQL = "select tradeacctno,b.type frg_type, " _
                    & "a.OPERATION_FIELD type, " _
                    & "a.micode,c.code_trade tradecode  , (CASE WHEN ( a.pro_enddate < get_t_plus(TO_DATE ('" & v_dCurrentDate & "','dd/mm/yyyy'), '0008', 1) ) THEN '0' ELSE a.is_pro_investor END) ISPRO " _
                    & "from rgiiia_cb a, rgii b , rgmi c " _
                    & "where(a.iicode = b.iicode And a.deleted = 0 And b.deleted = 0) " _
                    & "and a.micode =c.micode and c.deleted =0 " _
                    & "and a.micode NOT IN (SELECT d.micode FROM rgmi_cb_suspension d where d.deleted = 0)"

            v_strObjMsg = BuildXMLObjMsg(Now.Date, m_BusLayer.CurrentTellerProfile.BranchId, Now.Date, _
                    m_BusLayer.CurrentTellerProfile.TellerId, CommonLibrary.gc_IsLocalMsg, CommonLibrary.gc_MsgTypeObj, _
                    OBJNAME_SY_AUTHENTICATION, CommonLibrary.gc_ActionInquiry, v_strSQL, "RGII_INFO", "GetExportData")
        ...
        End Try
    End Function
    Private Function GetXMLCacBon_RGIIInfoData() As Xml.XmlDocument
        Try
            Dim v_strSQL As String = ""
            Dim v_xmlDocument As New Xml.XmlDocument
            Dim v_strObjMsg As String
            ...
            v_strSQL = "select tradeacctno, b.type frg_type, " _
                    & "a.micode, c.code_trade tradecode, decode(a.type_acc, 2, 3, a.type_acc) accounttype " _
                    & "from rgiiia_qs a, rgii b , rgmi c " _
                    & "where(a.iicode = b.iicode And a.deleted = 0 And b.deleted = 0) " _
                    & "and a.micode =c.micode and c.deleted =0 " _
                    & "and a.micode NOT IN (SELECT d.micode FROM rgmi_qs_suspension d where d.deleted = 0)"
            ...
            v_strObjMsgTemp = Replace(v_strObjMsgTemp, " />", ">")
            Dim v_strXMLHeader As String = "<?xml version=""1.0"" standalone=""yes"" ?> "
            Dim v_strRootOpen = "<HNX_CACBON  xmlns=""http://tempuri.org/HNX_CACBON.xsd"">"
            Dim v_strRootClose = "</HNX_CACBON>"
    ...
    Private Function GetXMLTransRjData() As Xml.XmlDocument
        Try
            Dim v_strSQL As String = ""
            Dim v_xmlDocument As New Xml.XmlDocument
            Dim v_strObjMsg As String

            v_strSQL = "SELECT b.col_value01  TRADING_DATE, " _
                        & "        b.col_value07 SICODE, " _
                        & "         c.isin, " _
                        & "         b.col_value03 CONFIRM_NO, " _
                        & "         b.col_value09 QUANTITY, " _
                        & "         b.col_value08 PRICE, " _
                        & "         a.col_value20 REASON " _
                        & "    FROM tllog a, tllog b , rgsi c " _
                        & "     WHERE a.tltxcd ='4122' AND a.status =3 " _
                        & "             AND a.brid ='" & BranchId & "' AND b.tltxcd ='4110' " _
                        & "             AND a.brid = b.brid AND b.txnum = a.col_value21 " _
                        & "             AND c.brid ='" & BranchId & "' AND c.sicode =b.COL_value07 " _
                        & "             AND c.DELeted =0 AND c.status =0"

            v_strObjMsg = BuildXMLObjMsg(Now.Date, m_BusLayer.CurrentTellerProfile.BranchId, Now.Date, _
                    m_BusLayer.CurrentTellerProfile.TellerId, CommonLibrary.gc_IsLocalMsg, CommonLibrary.gc_MsgTypeObj, _
                    OBJNAME_SY_AUTHENTICATION, CommonLibrary.gc_ActionInquiry, v_strSQL, "TRADING_REJECT", "GetExportData")
    ...
    End Function
    'end BOND Info TPRL
    Private Function GetXMLCacBon_TransRjData() As Xml.XmlDocument
        Try
            Dim v_strSQL As String = ""
            Dim v_xmlDocument As New Xml.XmlDocument
            Dim v_strObjMsg As String
            ...
            v_strSQL = "SELECT b.col_value01  TRADING_DATE, " _
                        & "        b.col_value07 CODE, " _
                        & "         b.col_value03 CONFIRM_NO, " _
                        & "         b.col_value09 QUANTITY, " _
                        & "         b.col_value08 PRICE, " _
                        & "         a.col_value20 REASON " _
                        & "    FROM tllog a, tllog b , rgsi c " _
                        & "     WHERE a.tltxcd ='4222' AND a.status =3 " _
                        & "             AND a.brid ='" & BranchId & "' AND b.tltxcd ='4210' " _
                        & "             AND a.brid = b.brid AND b.txnum = a.col_value21 " _
                        & "             AND c.brid ='" & BranchId & "' AND c.sicode =b.COL_value07 " _
                        & "             AND c.DELeted =0 AND c.status =0"
    ...
    Private Function GetXMLMIData() As Xml.XmlDocument
        ...
        End Try
    End Function
    Private Function GetXMLMIData_CACBON() As Xml.XmlDocument
        Try
            Dim v_strSQL As String = ""
            Dim v_xmlDocument As New Xml.XmlDocument
            Dim v_strObjMsg As String
            ...
            v_strSQL = "SELECT A.MICODE CODE, A.CODE_TRADE, A.NAME, A.TYPE, B.status STATUS, " _
                    & "decode (A.BORF_FLAG,5,2,1) DORF_FLAG, A.BORF_FLAG FROM RGMI A, RGMI_QS B WHERE A.micode = B.micode AND A.DELETED =0 AND B.STATUS =0 and A.BORF_FLAG in (1,3,4,5,7) "
            ...
            v_strObjMsgTemp = Replace(v_strObjMsgTemp, " />", ">")
            Dim v_strXMLHeader As String = "<?xml version=""1.0"" standalone=""yes"" ?> "
            Dim v_strRootOpen = "<CACBON_MEMBERS xmlns=""http://tempuri.org/CACBON_MEMBERS.xsd"">"
            Dim v_strRootClose = "</CACBON_MEMBERS>"
        ...
        End Try
    End Function
    Private Function GetXMLALLOWANCE_LIST_CACBON() As Xml.XmlDocument
        Try
            Dim v_strSQL As String = ""
            Dim v_xmlDocument As New Xml.XmlDocument
            Dim v_strObjMsg As String

            v_strSQL = "SELECT A.SICODE CODE, B.compliance_period ISSUEYEAR  FROM RGSI A, rgquota_info B WHERE A.sicode = B.sicode AND STOCK_TYPE = 1 AND a.type = 5 and A.brid = '" & BranchId & "' AND A.DELETED =0 AND A.STATUS =0 " _
            & "AND( A.EFFECTIVE_DATE<=get_t_plus(to_date('" & v_dCurrentDate & "','DD/MM/YYYY'),'" & BranchId & "',1)) "

            v_strObjMsg = BuildXMLObjMsg(Now.Date, m_BusLayer.CurrentTellerProfile.BranchId, Now.Date, _
                    m_BusLayer.CurrentTellerProfile.TellerId, CommonLibrary.gc_IsLocalMsg, CommonLibrary.gc_MsgTypeObj, _
                    OBJNAME_SY_AUTHENTICATION, CommonLibrary.gc_ActionInquiry, v_strSQL, "CACBONALLOWANCE_INFO", "GetExportData")
            ...
            ' Check neu khong co data => chi tao header
            If v_xmlDocument.DocumentElement.ChildNodes.Count = 0 Then
                v_strObjMsg = "<?xml version=""1.0"" standalone=""yes"" ?><HNX_CACBON xmlns=""http://tempuri.org/HNX_CACBON.xsd""></HNX_CACBON>"
                v_xmlDocument.LoadXml(v_strObjMsg)
                Return v_xmlDocument
            End If
        ...
        End Try
    End Function
    Private Function GetXMLCREDIT_LIST_CACBON() As Xml.XmlDocument
        Try
            Dim v_strSQL As String = ""
            Dim v_xmlDocument As New Xml.XmlDocument
            Dim v_strObjMsg As String

            v_strSQL = "SELECT DISTINCT A.SICODE CODE, b.isname ISNAME  FROM RGSI a, RGIS b WHERE a.iscode = b.iscode AND a.brid = '" & BranchId & "' AND STOCK_TYPE = 2 AND a.type = 5 AND a.DELETED =0 AND a.STATUS =0 " _
            & "AND( a.EFFECTIVE_DATE<=get_t_plus(to_date('" & v_dCurrentDate & "','DD/MM/YYYY'),'" & BranchId & "',1)) "

            v_strObjMsg = BuildXMLObjMsg(Now.Date, m_BusLayer.CurrentTellerProfile.BranchId, Now.Date, _
                    m_BusLayer.CurrentTellerProfile.TellerId, CommonLibrary.gc_IsLocalMsg, CommonLibrary.gc_MsgTypeObj, _
                    OBJNAME_SY_AUTHENTICATION, CommonLibrary.gc_ActionInquiry, v_strSQL, "CACBONCREDIT_INFO", "GetExportData")
        ...
        Try
            If pv_blnChecked Then
                Select Case Trim(pv_strFileName)
                    ...
                    Case "INFO"
                        v_xmlDocument = GetXMLSIBondCData()
                    Case "RGII_INFO"
                        v_xmlDocument = GetXMLRGIIInfoData()
                    Case "CACBON_RGII_INFO"
                        v_xmlDocument = GetXMLCacBon_RGIIInfoData()
                    Case "TRADING_REJECT" ' TPRL
                        v_xmlDocument = GetXMLTransRjData()
                    Case "CACBON_TRADING_REJECT" 'CacBon
                        v_xmlDocument = GetXMLCacBon_TransRjData()
                    Case "ISSUERS"
                    ...
                    Case "MEMBERS"
                        ...
                        '    v_xmlDocument.Save(v_strFileName)
                        'End If
                    Case "CACBON_MEMBERS" ' Cacbon
                        v_xmlDocument = GetXMLMIData_CACBON()
                    Case "CACBON_ALLOWANCE_INFO" ' Cacbon
                        v_xmlDocument = GetXMLALLOWANCE_LIST_CACBON()
                    Case "CACBON_CREDIT_INFO" 'Cacbon
                        v_xmlDocument = GetXMLCREDIT_LIST_CACBON()
                    Case "FOREIGN_ROOM"
            ...
            If Not v_xmlDocument Is Nothing Then
                ...
                Else
                    ...
                    'edit TPRL 
                    If v_Prefix = "HNX_BOND_CORP" Then
                        v_strFileName = pv_strFilePath & v_Prefix & "_" & pv_strFileName & v_dCurrentDateTPRL.ToString & ".xml"
                    ElseIf v_Prefix = "HNX" Then
                        v_strFileName = pv_strFilePath & v_Prefix & "_" & pv_strFileName & v_dCurrentDateCacbon.ToString & ".xml"
                    Else
                        v_strFileName = pv_strFilePath & v_Prefix & "_" & pv_strFileName & v_dCurrentDate.ToString & ".xml"
                    End If
    ...
    Private Sub OnSubmit()
        ...
        Try
            ...
            If tpStocks.Focus Then
                ...
                'bangpv add TPRL 
                v_blnExpIInfo = cbRi.Checked
                v_blnTransRj = cbTransRj.Checked
                'end add tprl 
                v_blnCacbonAllowance = cbQuota_CacBon.Checked
                v_blnCacbonCredit = cbCredit_CacBon.Checked
                v_blnExpIInfoCacbon = cbRi.Checked
                v_blnTransRjCacbon = cbTransReject_Cacbon.Checked
                v_blnExpMiCacbon = cbMi.Checked
                ...
                Else
                    ...
                    'bangpv : + cho TPRL 
                    ' 17/03/2026 Hoalx3 added cacbon
                    If BranchId = "0008" Then
                        ExportToXMLFile(v_blnExpSi, v_strFilePath, "INFO", v_prefix)
                        ExportToXMLFile(v_blnExpIInfo, v_strFilePath, "RGII_INFO", v_prefix)
                        ExportToXMLFile(v_blnTransRj, v_strFilePath, "TRADING_REJECT", v_prefix)
                    ElseIf BranchId = "0009" Then
                        ' 1. Danh sach ma han ngach duoc giao dich
                        ExportToXMLFile(v_blnCacbonAllowance, v_strFilePath, "CACBON_ALLOWANCE_INFO", v_prefix)
                        ' 2. Danh sach ma tin chi duoc giao dich
                        ExportToXMLFile(v_blnCacbonCredit, v_strFilePath, "CACBON_CREDIT_INFO", v_prefix)
                        ' 3. Thong tin nha dau tu/cac tai khoan luu ky dc phep giao dich
                        ExportToXMLFile(v_blnExpIInfoCacbon, v_strFilePath, "CACBON_RGII_INFO", v_prefix)
                        ' 5. Lenh huy thanh toan
                        ExportToXMLFile(v_blnTransRjCacbon, v_strFilePath, "CACBON_TRADING_REJECT", v_prefix)
                    Else
                        ExportToXMLFile(v_blnExpSi, v_strFilePath, "STOCKS", v_prefix)
                    End If
                    'end bangpv: TPRL 
                    If BranchId = "0009" Then
                        ExportToXMLFile(v_blnExpMiCacbon, v_strFilePath, "CACBON_MEMBERS", v_prefix)
                    Else
                    ...
                    If BranchId = "0002" Then
                        ...
                        '    End If
                        'End If
                    ElseIf BranchId = "0008" Then
                        mv_xZipEngine.ZipFileNotDel(v_strFilePath, v_prefix & v_dCurrentDateTPRL & ".zip")
                        If v_blnExpSi Then

                            File.Delete(v_strFilePath & v_prefix & "_INFO" & v_dCurrentDateTPRL & ".xml")
                        End If
                        If v_blnExpIs Then
                            File.Delete(v_strFilePath & v_prefix & "_ISSUERS" & v_dCurrentDateTPRL & ".xml")
                        End If
                        If v_blnExpMi Then
                            File.Delete(v_strFilePath & v_prefix & "_MEMBERS" & v_dCurrentDateTPRL & ".xml")
                        End If
                        If v_blnExpRoom Then
                            File.Delete(v_strFilePath & v_prefix & "_FOREIGN_ROOM" & v_dCurrentDateTPRL & ".xml")
                        End If
                        If v_blnExpIInfo Then
                            File.Delete(v_strFilePath & v_prefix & "_RGII_INFO" & v_dCurrentDateTPRL & ".xml")
                        End If
                        If v_blnTransRj Then
                            File.Delete(v_strFilePath & v_prefix & "_TRADING_REJECT" & v_dCurrentDateTPRL & ".xml")
                        End If
                    ElseIf BranchId = "0009" Then
                        v_prefix = "HNX_CACBON"
                        mv_xZipEngine.ZipFileNotDel(v_strFilePath, v_prefix & v_dCurrentDateCacbon & ".zip")
                        If v_blnExpIInfoCacbon Then

                            File.Delete(v_strFilePath & v_prefix & "_RGII_INFO" & v_dCurrentDateCacbon & ".xml")
                        End If
                        If v_blnExpIs Then
                            File.Delete(v_strFilePath & v_prefix & "_ISSUERS" & v_dCurrentDateCacbon & ".xml")
                        End If
                        If v_blnExpMiCacbon Then
                            File.Delete(v_strFilePath & v_prefix & "_MEMBERS" & v_dCurrentDateCacbon & ".xml")
                        End If
                        If v_blnCacbonCredit Then
                            File.Delete(v_strFilePath & v_prefix & "_CREDIT_INFO" & v_dCurrentDateCacbon & ".xml")
                        End If
                        If v_blnCacbonAllowance Then
                            File.Delete(v_strFilePath & v_prefix & "_ALLOWANCE_INFO" & v_dCurrentDateCacbon & ".xml")
                        End If
                        If v_blnTransRjCacbon Then
                            File.Delete(v_strFilePath & v_prefix & "_TRADING_REJECT" & v_dCurrentDateCacbon & ".xml")
                        End If
                    Else
                        ...
                        If v_blnExpSi Then
                            ...
                            ' File.Delete(v_strFilePath & v_prefix & "_INFO_" & v_dCurrentDateTPRL & ".xml")
                        End If
                        If v_blnExpIs Then
...
#End Region
    ...
    Private Sub cboBranch_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboBranch.SelectedIndexChanged
        ...
        Else
            ...
            cbIs.Enabled = True
        End If
        If Trim(cboBranch.SelectedValue.ToString) = "0008" Then
            cbRi.Visible = True
            cbTransRj.Visible = True
            cbRi.Text = "Thông tin nhà đầu tư giao dịch TPRL"
        ElseIf Trim(cboBranch.SelectedValue.ToString) = "0009" Then
            cbRi.Visible = True
            cbRi.Text = "Thông tin nhà đầu tư được phép giao dịch Các-bon"
            cbTransRj.Visible = False
        Else
            cbTransRj.Visible = False
            cbRi.Visible = False
        End If
        'hoalx3 add CacBon
        If Trim(cboBranch.SelectedValue.ToString) = "0009" Then
            cbTransReject_Cacbon.Visible = True
            cbCredit_CacBon.Visible = True
            cbQuota_CacBon.Visible = True
            ...
            cbIs.Checked = False
        Else
            cbTransReject_Cacbon.Visible = False
            cbCredit_CacBon.Visible = False
            cbQuota_CacBon.Visible = False
            cbRoom.Visible = True
            cbRoom.Enabled = True
            cbSi.Enabled = True
            cbIs.Enabled = True
