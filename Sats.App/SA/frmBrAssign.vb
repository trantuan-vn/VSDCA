...
#Region "Method"
    ...
    Private Sub LoadUserInterface(ByRef pv_ctrl As Windows.Forms.Control)
        ...
        Try
            ...
            Me.Text = mv_ResourceManager.GetString("frmBrAssign")
            Dim v_strCaption As String
            If AssignType = "User" Then
                v_strCaption = Replace(mv_ResourceManager.GetString("lbCaption2"), "$", "NSD")
            Else
                v_strCaption = Replace(mv_ResourceManager.GetString("lbCaption2"), "$", " nhóm")
            End If
    ...
    Private Sub FillData()
        ...
        Dim v_strBrID, v_strDisplay As String
        ...
        Try
            ...
            mv_strMIOutUser = ""
            mv_strMIInUser = ""
            'Lấy mã chi nhánh chưa được phân quyền
            v_strCmdInquiryUsrOutGrp = ""

            v_strCmdInquiryUsrOutGrp = "SELECT BRID VALUE, BRID || ' - ' || BRNAME display" _
                                            & " FROM brgrp WHERE brid NOT IN (SELECT brid FROM tlbridauth" _
            ...
            For i As Integer = 0 To v_nodeOutGrpList.Count - 1
                ...
                If Not hTellerFilter(v_strBrID) Is Nothing Then
                    mv_strMIOutUser &= v_strBrID & "|"
    ...
    Private Sub FillUserData()
        ...
        Try
            ...
            mv_strMIOutUser = ""
            mv_strMIInUser = ""
            'Lay mã chi nhánh chua dc phan quyen cho nhom hoac NSD
            v_strCmdInquiryUsrOutGrp = ""

            v_strCmdInquiryUsrOutGrp = "SELECT brid VALUE, brid || ' - ' || BRNAME display" _
                                            & " FROM BRGRP WHERE BRID NOT IN(" _
                                            & " SELECT  DISTINCT m.BRID" _
                                            & " FROM brgrp m, tlbridauth a" _
                                            ...
                                            & " And m.deleted = 0 And m.brid = a.brid) AND deleted=0" _
                                            & " ORDER BY VALUE"
            ...
            ''''==== Select users' name that are in group ====''''
            'Lay mã chi nhánh da phan quyen cho nhom hoac NSD
            v_strCmdInquiryUserInGroup = "SELECT  DISTINCT m.brid VALUE, m.brid || ' - ' || BRNAME display, a.authtype" _
                                        & " FROM brgrp m, tlbridauth a" _
                                        & " WHERE ((a.AUTHID = '" & GroupId & "' AND a.authtype = 'U')" _
                                        & " OR (a.AUTHID IN (SELECT grpid FROM tlgrpusers" _
                                        & " WHERE tlid = '" & GroupId & "') AND a.authtype = 'G'))" _
                                        & " And m.deleted = 0 And m.brid = a.brid" _
                                        & " ORDER BY M.brid"
            ...
            For i As Integer = 0 To v_nodeInGrpList.Count - 1
                ...
                If Not hTellerFilter(v_strBrID) Is Nothing Then
                    If v_strBrID <> v_strtmpSICode Then
                        lstUserInGroup.Items.Add(v_strDisplay)
                    End If
                    If v_strAuthType = "U" Then
                        mv_strMIInUser &= v_strBrID & "|"
                    Else
                        hTlidInGrpFilter.Add(v_strBrID, v_strBrID)
                    End If
                End If
                v_strtmpSICode = v_strBrID
            Next
    ...
    Private Sub FillTellerData()
        ...
        Try
            ...
            v_strUsrOutGrpObjMsg = BuildXMLObjMsg(Now.Date, m_BusLayer.CurrentTellerProfile.BranchId, Now.Date, _
                    ...
                    OBJNAME_SY_AUTHENTICATION, CommonLibrary.gc_ActionInquiry, , m_BusLayer.CurrentTellerProfile.TellerId, "GetTellerBrIDData")
    ...
    Private Sub OnSave()
        ...
        Try
            ...
            If AssignType = "Group" Then
                v_strObjMsg = BuildXMLObjMsg(, , , TellerId, gc_IsNotLocalMsg, gc_MsgTypeObj, OBJNAME_SA_TLGROUPS, gc_ActionAdhoc, , v_strClause, "BrIDAssign")
            Else
                v_strObjMsg = BuildXMLObjMsg(, , , TellerId, gc_IsNotLocalMsg, gc_MsgTypeObj, OBJNAME_SA_TLPROFILES, gc_ActionAdhoc, , v_strClause, "BrIDAssign")
            End If
    ...
    Private Sub ReMoveOne()
        Try
            ...
            If Not hTlidInGrpFilter(v_strSelect) Is Nothing Then
                MsgBox(mv_ResourceManager.GetString("RemoveErr2") & v_strSelect, MsgBoxStyle.Information + MsgBoxStyle.OkOnly, gc_ApplicationTitle)
                Exit Sub
            End If
        ...
        Try
            ...
            If AssignType = "User" Then
                ...
                If v_strMsg <> "" Then
                    ...
                    MsgBox(mv_ResourceManager.GetString("RemoveErr2") & v_strMsg, MsgBoxStyle.Information + MsgBoxStyle.OkOnly, gc_ApplicationTitle)
                End If
            End If
        Catch ex As Exception
            LogError.Write("Error source: " & ex.Source & vbNewLine _
...
#Region "Sự kiện Form"
    ...
    Private Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        If Not lstUserNoGroup.SelectedItems.Count < 1 Then
            Me.MoveOne()
        Else
            MsgBox(mv_ResourceManager.GetString("NothingSelected2"), MsgBoxStyle.Information + MsgBoxStyle.OkOnly, gc_ApplicationTitle)
        End If
