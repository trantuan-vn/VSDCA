...
Public Class frmSysVarSetting
    ...
    Private Const TPRL_BRANCH As String = "0008"
    Private Const CACBON_BRANCH As String = "0009"
    ...
    'end
    Dim mv_htbListDateTPRL As New Hashtable
    Dim mv_htbListDateCACBON As New Hashtable
    ...
    'end
    Dim mv_lstListDateTPRL As New List(Of String)
    Dim mv_lstListDateCACBON As New List(Of String)
...
#Region "Method"
    ...
    Private Sub FillDataCombobox()
        Try
            Dim v_strObjMsg As String
            Dim strRow As String = "SELECT BRID VALUE, BRID || ' - ' || BRNAME DISPLAY FROM BRGRP WHERE STATUS = 0 AND DELETED = 0 AND BRID IN ('0001','0002','0003', '0004', '0005','0006','0007', '0008', '0009') ORDER BY BRID"
    ...
    Private Sub GetListDate()
        Try
            ...
            'Thanglv9-add them thi truong 0007 và 0008
            mv_htbListDateDLCN.Clear()
            mv_htbListDateTPRL.Clear()
            mv_htbListDateCACBON.Clear()
            ...
            'End
            mv_lstListDateTPRL.Clear()
            mv_lstListDateCACBON.Clear()
            ...
            For i As Integer = 0 To v_nodeList.Count - 1
                ...
                Select Case v_strBrId
                    ...
                    Case DLCN_BRANCH
                        ...
                        'end
                    Case TPRL_BRANCH
                        mv_htbListDateTPRL.Add(v_strSbDate, v_strAutoId)
                        mv_lstListDateTPRL.Add(v_strSbDate)
                    Case CACBON_BRANCH
                        mv_htbListDateCACBON.Add(v_strSbDate, v_strAutoId)
                        mv_lstListDateCACBON.Add(v_strSbDate)
                End Select
            Next
        Catch ex As Exception
            Throw ex
    ...
    Private Sub UpdateSA()
        ...
        Try
            ...
            'end
            v_strClause &= GetStrUpdateSA(mv_htbListDateTPRL, mv_lstListDateTPRL, TPRL_BRANCH) & "$"
            v_strClause &= GetStrUpdateSA(mv_htbListDateCACBON, mv_lstListDateCACBON, CACBON_BRANCH)
        ...
        Try
            Select Case BRID
                ...
                Case DLCN_BRANCH
                    ...
                    'end
                Case TPRL_BRANCH
                    mv_lstListDateTPRL.Remove(lstListDate.SelectedItem)
                    lstListDate.Items.Remove(lstListDate.SelectedItem)
                Case CACBON_BRANCH
                    mv_lstListDateCACBON.Remove(lstListDate.SelectedItem)
                    lstListDate.Items.Remove(lstListDate.SelectedItem)
            End Select
        Catch ex As Exception
            Throw ex
        ...
        Try
            ...
            Select Case BRID
                ...
                Case DLCN_BRANCH
                    ...
                    'end
                Case TPRL_BRANCH
                    If mv_lstListDateTPRL.Contains(Trim(v_dDate.ToString)) Then
                        MsgBox(mv_ResourceManager.GetString("DateExist"), MsgBoxStyle.Information + MsgBoxStyle.OkOnly, gc_ApplicationTitle)
                    Else
                        mv_lstListDateTPRL.Add(v_dDate.ToString)
                        lstListDate.Items.Add(Trim(v_dDate.ToString))
                    End If
                Case CACBON_BRANCH
                    If mv_lstListDateCACBON.Contains(Trim(v_dDate.ToString)) Then
                        MsgBox(mv_ResourceManager.GetString("DateExist"), MsgBoxStyle.Information + MsgBoxStyle.OkOnly, gc_ApplicationTitle)
                    Else
                        mv_lstListDateCACBON.Add(v_dDate.ToString)
                        lstListDate.Items.Add(Trim(v_dDate.ToString))
                    End If
            End Select
        Catch ex As Exception
...
#End Region
        ...
        If Not cboBranchId.SelectedValue Is Nothing Then
            ...
            Select Case BRID
                ...
                Case DLCN_BRANCH
                    ...
                    'end
                Case TPRL_BRANCH
                    FillListDate(mv_lstListDateTPRL, Month, Year)
                Case CACBON_BRANCH
                    FillListDate(mv_lstListDateCACBON, Month, Year)
            End Select
        End If
        System.Threading.Thread.CurrentThread.CurrentCulture = System.Globalization.CultureInfo.GetCultureInfo("en-US")
    End Sub
    Private Sub btnInitAll_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnInitAll.Click
        ...
        If IsNumeric(v_strYear) Then
            ...
            If v_intYear >= 2000 And v_intYear <= 3020 Then
