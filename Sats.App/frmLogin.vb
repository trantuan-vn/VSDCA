...
Public Class frmLogin
    ...
    Private Sub SelectBranch()
        ...
        'Check chan voi san cacbon 0009
        'Check ma Tin chi, Han ngach huy nhung chua duoc xu ly
        If m_BusLayer.CurrentTellerProfile.BranchId = "0009" Then
            Dim v_strSQL As String
            Dim v_strObjMsg As String
            v_strSQL = "SELECT LISTAGG(SICODE, ',') WITHIN GROUP (ORDER BY SICODE) AS SICODE_LIST FROM RGSI A WHERE STOCK_TYPE in ('1', '2') AND a.type = 5 and A.brid = '" & m_BusLayer.CurrentTellerProfile.BranchId & "' AND A.DELETED =0 AND A.STATUS =0 " _
                    & "AND( A.due_date = get_t_plus(to_date('" & m_BusLayer.CurrentTellerProfile.BusDate & "','DD/MM/YYYY'),'" & m_BusLayer.CurrentTellerProfile.BranchId & "',0))"
            ...
            Dim v_sicodeList_DueDate = v_xml.SelectSingleNode("/ObjectMessage/ObjData/entry[@fldname='SICODE_LIST']").InnerText
            If Len(Trim(v_sicodeList_DueDate)) > 0 Then
                MsgBox("Danh sách mã tín chỉ, hạn ngạch cần hủy trong ngày: '" & v_sicodeList_DueDate & "'!", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly, gc_ApplicationTitle)
            End If
        End If
