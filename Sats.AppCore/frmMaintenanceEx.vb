...
Public Enum SaveButtonTypeEx
    OKButton = 0
    ApplyButton = 1
    CancelButton = 2
End Enum

Public Class frmMaintenanceEx
...
#Region " Overridable Functions "
    ...
    Public Overridable Sub FillData()
        ...
        Try
            ...
            Dim v_strCmdInquiry As String = "SELECT " & v_strlstField & " FROM " & TableName & " WHERE " & v_strFilter & " AND DELETED=0"
...
#Region " Private Functions "
    ...
    Private Sub LoadObjectFields()
        ...
        Dim v_arrTFldMaster() As TFldMaster
        Try
            v_arrTFldMaster = LinqEx.GetFldMasterList(ObjectName)
            ReDim mv_arrObjFields(v_arrTFldMaster.Length)

            For i As Integer = 0 To v_arrTFldMaster.Length - 1
                Dim v_objField As New CFieldMaster
                With v_objField
                    .FieldName = v_arrTFldMaster(i).FLDNAME
                    .ColumnName = v_arrTFldMaster(i).DEFNAME
                    .Caption = v_arrTFldMaster(i).CAPTION
                    .EnCaption = v_arrTFldMaster(i).EN_CAPTION
                    .DisplayOrder = CInt(v_arrTFldMaster(i).ODRNUM)
                    .FieldType = v_arrTFldMaster(i).FLDTYPE
                    .InputMask = v_arrTFldMaster(i).FLDMASK
                    .FieldFormat = v_arrTFldMaster(i).FLDFORMAT
                    .FieldLength = v_arrTFldMaster(i).FLDLEN
                    .LookupList = v_arrTFldMaster(i).LLIST
                    .LookupCheck = v_arrTFldMaster(i).LCHK
                    .DefaultValue = v_arrTFldMaster(i).DEFVAL
                    .Visible = (Trim(v_arrTFldMaster(i).VISIBLE) = "Y")
                    .Enabled = (Trim(v_arrTFldMaster(i).DISABLE) = "N")
                    .Mandatory = (Trim(v_arrTFldMaster(i).MANDATORY) = "Y")
                    .AmtExp = v_arrTFldMaster(i).AMTEXP
                    .ValidTag = v_arrTFldMaster(i).VALIDTAG
                    .LookUp = v_arrTFldMaster(i).LOOKUP
                    .DataType = v_arrTFldMaster(i).DATATYPE
                    .LookupName = v_arrTFldMaster(i).LOOKUPNAME
                    .SearchCode = v_arrTFldMaster(i).SEARCHCODE
                    .SrModCode = v_arrTFldMaster(i).SRMODCODE
                    .RiskField = v_arrTFldMaster(i).RISKFLD
                End With
                mv_arrObjFields(i) = v_objField
            Next

            ReDim Preserve mv_arrObjFields(v_arrTFldMaster.Length)
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Private Sub LoadObjectFieldValidateRules()
        ...
        Dim v_arrFldVal() As TFldVal
        Try
            'Lấy các luật kiểm tra của các trường giao dịch
            v_arrFldVal = LinqEx.GetFldValList(ObjectName)
            ReDim mv_arrObjFldVals(v_arrFldVal.Length)

            Dim v_strFieldVal_FldName As String = ""
            Dim v_strFieldVal_ValType As String = ""
            Dim v_strFieldVal_Operator As String = ""
            Dim v_strFieldVal_ValExp As String = ""
            Dim v_strFieldVal_ValExp2 As String = ""
            Dim v_strFieldVal_ErrMsg As String = ""
            Dim v_strFieldVal_EnErrMsg As String = ""

            For i As Integer = 0 To v_arrFldVal.Length - 1
                'Ghi nhận thuật toán để kiểm tra và tính toán cho từng trường của form
                v_strFieldVal_FldName = Trim(v_arrFldVal(i).FLDNAME)
                v_strFieldVal_ValType = Trim(v_arrFldVal(i).VALTYPE)
                v_strFieldVal_Operator = Trim(v_arrFldVal(i).OPERATOR_)
                v_strFieldVal_ValExp = Trim(v_arrFldVal(i).VALEXP)
                v_strFieldVal_ValExp2 = Trim(v_arrFldVal(i).VALEXP2)
                v_strFieldVal_ErrMsg = Trim(v_arrFldVal(i).ERRMSG)
                v_strFieldVal_EnErrMsg = Trim(v_arrFldVal(i).EN_ERRMSG)
            ...
            ReDim Preserve mv_arrObjFldVals(v_arrFldVal.Length)
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
                    ...
                    For i As Integer = 0 To UBound(mv_arrObjFields) - 1
                        With mv_arrObjFields(i)
                            If (v_ctrl.Tag = .FieldName) And (.FieldType = "C") And (.LookupList.Length > 0) Then
                                ...
                                Else
                                    Dim v_ds As DataSet
                                    If v_strSQL.StartsWith(":") Then
                                        Dim v_strCdType As String = Mid(v_strSQL, 2).Split(".")(0)
                                        Dim v_strCdName As String = Mid(v_strSQL, 2).Split(".")(1)

                                        v_ds = LinqEx.GetAllCode(v_strCdType, v_strCdName)
                                        'Fill du lieu vao combobox
